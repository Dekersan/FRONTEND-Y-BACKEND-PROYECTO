...
@app.post('/guia/crear')
async def ValidacionGuias(guia_data: guia):
    try:
        ...
        raise HTTPException(status_code=500, detail="Error interno del servidor al procesar la solicitud. Detalle: {}".format(str(e)))
...
@app.options('/usuario/revisar')
async def options_usuario_revisar():
    return {}


@app.post('/usuario/revisar', response_model=dict)
async def revisar_usuario(usuario: Usuario):
    try:
      
        connection = get_connection()  
        cursor = connection.cursor() 
        select_query = "SELECT COUNT(*) FROM usuarios WHERE Correo = %s AND Contraseña = %s"
        usuarios_values = (usuario.Correo, usuario.Contraseña)
        cursor.execute(select_query, usuarios_values)
        count = cursor.fetchone()[0]

        if count > 0:
        
            return {"mensaje": "El usuario existe en la base de datos", "redireccionar": True}
        else:
            
            return {"mensaje": "El usuario no existe en la base de datos", "redireccionar": False}
    except Exception as e:
        raise HTTPException(status_code=500, detail="Error interno del servidor al procesar la solicitud")
